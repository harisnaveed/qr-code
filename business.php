<!doctype html>
<html lang="en">
	<head>
		<!-- Required meta tags -->
		<meta charset="utf-8">
		<!-- Bootstrap CSS -->
		<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
		<link href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css" rel="stylesheet" integrity="sha384-wvfXpqpZZVQGK6TAh5PVlGOfQNHSoD2xbE+QkPxCAFlNEevoEH3Sl0sibVcOQVnN" crossorigin="anonymous">
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
		<link rel="stylesheet" type="text/css" href="css/style.css">
		<link rel="stylesheet" type="text/css" href="css/footer_style.css">

		<title> Online QR code generator for business and download QR image | QR-setup </title>
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<meta name="keywords" content="Online , qr  code, generator, QR SETUP, download, QR image, business"/>
		<meta name="description" content="QR setup is here to provide you the best services of QR code generators. In this platform you can create unlimited non expiry QR code for business and download QR images for free. The whole process includes simple three steps, insert ,generate, download. No sign up  and sign in required."/>
		<meta name="copyright"content="qr-setup">
		<meta name="language" content="ES">
		<meta name="robots" content="index,follow"/>
		<meta name="googlebot" content="index">
		<meta name="author" content="Umer Shahzad,umer.shahzad.softwareengineer@gmail.com">
		<meta name="url" content="https://qr-setup.com/business.php">
		<meta name="owner" content="QR-Setup">
		<meta name="Classification" content="Business">
		<link rel="canonical" href="https://qr-setup.com/business.php">
		<meta name="twitter:card" content="QR-Setup - Generate QR code For your business"/>
		<link rel="shortcut icon" href="img/icon.png"/>
	</head>
	<body onload="start()">
		
			<nav id="navbar"class="navbar navbar1 navbar-expand-lg bg-white my-3">
			<a href="index.php"><img class="logo" src="img/logo.png" alt="Logo"></a>
			<ul class="navbar-nav ml-auto">
				<li class="nav-item">
					<a href="generate-qr-code.php" class="nav-btn">Generate Qr</a>
				</li>
		    </ul>
		    </nav>
		
		
		<h1 class="heading text-center mt-3">QR for Business</h1>

		<div class="nav">
			<div class="links">
				<a onclick="product(this),qrNone()" id="product">Product/Packaging</a>
				<a onclick="news(this),qrBlock()" id="news">News</a>
				<a onclick="application(this),qrNone()" id="application">Application Form</a>
				<a onclick="tickets(this),qrBlock()" id="tickets">Tickets</a>
				<a onclick="events(this),qrBlock()" id="events">Events</a>
				<a onclick="marketing(this),qrNone()" id="marketing">Marketing Items</a>
				<a onclick="cards(this),qrNone()" id="cards">Cards</a>
			</div>
		</div>
		
		            <!--  Cards display or subcategories -->
                    <div id="demo"></div>

		    <div class="container my-5">
		    <div class="row">
		        		<div class="col-xl-3 col-lg-3 col-md-4 col-sm-12" >
		        		</div>
		        <?php
     			 if(isset($_GET['qrimage'])){
      			   $imgg = $_GET['qrimage'];
      		    ?>
				<div class="col-xl-3 col-lg-3 col-md-4 col-sm-12 mobo" id="qr-img-div">
					<div class="qr-img-div">
						<img src="<?php echo $imgg; ?>" alt="Generated qr-image" class="qr-img">
					</div>
					<div class="download-btn-div">
						<a class="download-btn" href="<?php echo $imgg; ?>" download="qr-setup.png">Download Qr Code</a>
					</div>
				</div>
				<?php  
     			 }
     			 else{}
     		    ?>
     					<div class="col-xl-3 col-lg-3 col-md-4 col-sm-12" >
		        	    </div> 
		    </div>
		    <br>
			<div class="row">
				<div class="col-xl-9 col-lg-9 col-md-8 col-sm-12">
					
					<form method="POST" onsubmit="return validate()" action="functions.php" id="form">
				
					</form>
					
				</div>
				<!-- <div class="col-xl-2 d-lg-2 d-md-none "></div> -->
				
				<?php
     			 if(isset($_GET['qrimage'])){
      			   $imgg = $_GET['qrimage'];
      		    ?>
				<div class="col-xl-3 col-lg-3 col-md-4 col-sm-12 desktopo" id="qr-img-div">
					<div class="qr-img-div">
						<img src="<?php echo $imgg; ?>" alt="Generated qr-image" class="qr-img">
					</div>
					<div class="download-btn-div">
						<a class="download-btn" href="<?php echo $imgg; ?>" download="qr-setup.png">Download Qr Code</a>
					</div>
				</div>
				<?php  
     			 }
     			 else{
      			}
     				 ?>
			</div>
			
		</div>

		<?php 
        include("footer.php");
		 ?>
		<!-- Optional JavaScript -->
		<!-- jQuery first, then Popper.js, then Bootstrap JS -->
		<script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
		<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
		<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>

		<script src="js/business.js"type="text/javascript"></script>
		<script src="js/regexp.js"type="text/javascript"></script>
		<script src="js/business_validation.js"type="text/javascript"></script>
		
		<script type="text/javascript">
			function func(x){
				var menuitem=document.querySelectorAll('a');
				for(var i=0;i<menuitem.length;i++)
				{
					if(menuitem[i].className=='active')
						menuitem[i].classList.remove("active");
				}
				x.classList.add("active");
			}

			function qrNone()
			{
				document.getElementsByClassName("desktopo")[0].style.display = "none";
			}
			function qrBlock()
			{
				document.getElementsByClassName("desktopo")[0].style.display = "block";
			}

        function start(){
              
            <?php
                
                if(isset($_GET['value'])){
                   $value=$_GET['value'];
                   
                    if($value=="business-card")
                    { ?>
                    var x=document.getElementById("cards");
                    func(x);	
                    business();
                    <?php }

                    else if($value=="label")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);	
                    label();
                    <?php }

                    else if($value=="flyer")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);	
                    flyer();
                    <?php }

                    else if($value=="brochure")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);	
                    brochure();
                    <?php }
                    
                    else if($value=="product")
                    { ?>
                    var x=document.getElementById("product");
                    product(x);
                    <?php }
                    
                    else if($value=="news")
                    { ?>
                    var x=document.getElementById("news");
                    news(x);
                    <?php }

                    else if($value=="promotion")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);
                    promotion();
                    <?php }

                    else if($value=="coupon")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);
                    coupon();
                    <?php }

                    else if($value=="application")
                    { ?>
                    var x=document.getElementById("application");
                    application(x);
                    <?php }

                    else if($value=="tickets")
                    { ?>
                    var x=document.getElementById("tickets");
                    tickets(x);
                    <?php }

                    else if($value=="events")
                    { ?>
                    var x=document.getElementById("events");
                    events(x);
                    <?php }

                    else if($value=="marketing")
                    { ?>
                    var x=document.getElementById("marketing");
                    marketing(x);
                    <?php }

                    else if($value=="cards")
                    { ?>
                    var x=document.getElementById("cards");	
                    cards(x);
                    <?php }
                    //-------------------------------------------------------------------------------------
                    //    these checks for when data fetch from QRbarcode page to this page // 

                    else if($value=="prodPack")
                    { ?>
                    var x=document.getElementById("product");
                    func(x);	
                    prodPack();
                    <?php }

                    else if($value=="prod")
                    { ?>
                    var x=document.getElementById("product");
                    func(x);	
                    prod();
                    <?php }

                    else if($value=="employment")
                    { ?>
                    var x=document.getElementById("application");
                    func(x);	
                    employment();
                    <?php }

                    else if($value=="rental")
                    { ?>
                    var x=document.getElementById("application");
                    func(x);	
                    rental();
                    <?php }

                    else if($value=="member")
                    { ?>
                    var x=document.getElementById("application");
                    func(x);	
                    member();
                    <?php }

                    else if($value=="job")
                    { ?>
                    var x=document.getElementById("application");
                    func(x);	
                    job();
                    <?php }

                    else if($value=="ad")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);	
                    advertisement();
                    <?php }

                    else if($value=="banner")
                    { ?>
                    var x=document.getElementById("marketing");
                    func(x);	
                    banner();
                    <?php }

                    else if($value=="student-card")
                    { ?>
                    var x=document.getElementById("cards");
                    func(x);	
                    student();
                    <?php }

                    else if($value=="entry-card")
                    { ?>
                    var x=document.getElementById("cards");
                    func(x);	
                    entry();
                    <?php }

                    else if($value=="employ-card")
                    { ?>
                    var x=document.getElementById("cards");
                    func(x);	
                    employ();
                    <?php }

                    else        
                    { ?>
	                var x=document.getElementById("product");	
	                product(x);
	                <?php }


                    }

                else
                { ?>
                var x=document.getElementById("product");	
                product(x);
                <?php }
            ?>	
        }

		
		</script>
	</body>
</html>