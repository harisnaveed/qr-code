...
<html lang="en">
	<head>
		...
		<title> How to generate free qr code for promotion & ads and download free Qr image| QR-setup </title>
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<meta name="keywords" content="Generate, free, qr code ,promotion, ads ,download, free,QR image, qr-setup"/>
		<meta name="description" content="Qr setup provides you the Best generator to create QR code for promotion & ads. You can Just enter your data and your QR code is ready to download a QR image  .you can use this QR code anywhere any time. The whole process is not required to sign up and sign in"/>
		...
		<meta name="url" content="https://qr-setup.com/advertisement.php">
		<meta name="owner" content="QR-Setup">
		<meta name="Classification" content="Business">
		<link rel="canonical" href="https://qr-setup.com/advertisement.php">
	...
	<body onload="start()">
		...
		<div class="nav">
			<div class="links">
				...
				<a onclick="news(this),qrBlock()" id="news" >News</a>
				<a onclick="application(this),qrNone()" id="application" >Application Form</a>
				<a onclick="tickets(this),qrBlock()" id="tickets" >Tickets</a>
				<a onclick="events(this),qrBlock()" id="events" >Events</a>
        ...
        function start(){
                document.getElementById("demo").style.display = "none";
                   ...
                   var codeBlock='<div class="head">'+
						'<i class="icon fa fa-file-text-o" aria-hidden="true"></i><b class="head-text">   Advertisement</b>'+
						
					'</div>'+
					'<div class="packaging-div">'+
							'<label class=" add-title-label">Title of Advertisement:</label>'+
							'<input class="add-title-input" type="text" name="add_title" id="add_title" placeholder="Planet fitness, subway"  autocomplete="off" required="required"> <br>'+
							'<div class="error-message-div">'+
					           '<span class="error-message" id="error-message-add-title"></span>'+
                            '</div>'+
							
							'<label class=" add-heading1-label">Heading 1:</label>'+
							'<input class="add-heading1-input" type="text" name="add_h1" id="add_h1" placeholder="Train Hard"  autocomplete="off"> <br>'+
							'<div class="error-message-div">'+
					           '<span class="error-message" id="error-message-add-h1"></span>'+
                            '</div>'+
							
							'<label class=" add-heading2-label">Heading 2:</label>'+
							'<input class="add-heading2-input" type="text" name="add_h2" id="add_h2" placeholder="Eating Fresh"  autocomplete="off"> <br>'+
							'<div class="error-message-div">'+
					           '<span class="error-message" id="error-message-add-h2"></span>'+
                            '</div>'+
							
							'<label class=" add-budget-label"> Budget Detail:</label>'+
							'<input class="add-budget-input" type="text" name="add_budget" id="add_budget" placeholder="Enter the Budget detail"  autocomplete="off" required="required"> <br>'+
							'<div class="error-message-div">'+
					           '<span class="error-message" id="error-message-add-detail"></span>'+
                            '</div>'+
							
					'</div>'+
					'<div class="generate-btn-div">'+
						'<input class="generate-btn" type="submit" value="Generate Qr Code" name="business_advertisement" onclick="advertisment_validate()"/>'+
					'</div>';
